...
const loadData = async () => {
  if (import.meta.env.MODE === 'development') {
    ...
    const characterStr = localStorage.getItem('better-miyoushe-character-list')
    characterData = JSON.parse(characterStr || '[]')
  }
  vueData.value = [...characterData]
}
...
const MANE_IDS = [10000117, 10000118]

const TRAVELER_ID = [10000007]

// 奇偶角色置底，排序函数
const bottomSorter = (item) => MANE_IDS.includes(item.id) ? 1 : 0

const bottomTraveler = (item) => TRAVELER_ID.includes(item.id) ? 1 : 0

// 包装现有排序函数
const customSort = (fields, orders) => {
  return (data) => _.orderBy(data, [bottomSorter, ...fields], ['asc', ...orders])
};

const resetData = () => {
  vueData.value = _.orderBy([...characterData], bottomSorter, 'asc')
}
const sortById = () => {
  vueData.value = customSort(['id'], ['desc'])(vueData.value)
}
const sortByLevel = () => {
  vueData.value = customSort(['level_current'], ['desc'])(vueData.value)
}
const sortByFetter = () => {
  vueData.value = customSort([bottomTraveler, 'fetter_level', 'id'], ['asc', 'asc', 'desc'])(vueData.value)
}
const sortByCons = () => {
  vueData.value = customSort(
...
<template>
  <div class="container">
    ...
    <div class="container-table-row" v-for="(item, index) in vueData" :key="index"
...
@font-face {
  font-family: 'Number';
  src: url('https://raw.githubusercontent.com/yoimiya-kokomi/miao-plugin/refs/heads/master/resources/common/font/tttgbnumber.woff') format('woff'),
    url('https://raw.githubusercontent.com/yoimiya-kokomi/miao-plugin/refs/heads/master/resources/common/font/tttgbnumber.ttf') format('truetype');
}
